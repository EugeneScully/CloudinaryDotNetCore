...
namespace CloudinaryDotNet.Test
{
    public partial class ApiTest
    {
        ...
        public void WithLiteral()
        {
            ...
            Assert.Equal(sTransform.IndexOf("if"), 0);//, "should include the if parameter as the first component in the transformation string");
            Assert.Equal("if_w_lt_200,c_fill,h_120,w_80", sTransform);//, "should be proper transformation string");

            transformation = new Transformation().Crop("fill").Height(120).IfCondition("w_lt_200").Width(80);
            sTransform = transformation.ToString();
            Assert.Equal(sTransform.IndexOf("if"), 0);//, "should include the if parameter as the first component in the transformation string");
            Assert.Equal("if_w_lt_200,c_fill,h_120,w_80", sTransform);//, "components should be in proper order");
            ...
            transformation = new Transformation().IfCondition("w_lt_200").Crop("fill").Height(120).Width(80).
                                          ...
                                          Chain().Effect("sepia");
            sTransform = transformation.ToString();
            Assert.Equal("if_w_lt_200,c_fill,h_120,w_80/if_w_gt_400,c_fit,h_150,w_150/e_sepia", sTransform);//, "should allow multiple conditions when chaining transformations");
        }
        ...
        public void LiteralWithSpaces()
        {
            ...
            string sTransform = transformation.ToString();
            Assert.Equal("if_w_lt_200,c_fill,h_120,w_80", sTransform);//, "should translate operators");
        }
        ...
        public void EndIf()
        {
            ...
            Assert.Equal("if_w_lt_200/c_fill,e_sharpen,h_120,w_80/e_brightness:50/co_red,e_shadow/if_end", sTransform);//, "should be proper transformation string");
        }
        ...
        public void IfElse()
        {
            ...
            Assert.Equal("if_w_lt_200,c_fill,h_120,w_80/if_else,c_fill,h_90,w_100", sTransform);//, "should support if_else with transformation parameters");
            ...
            Assert.Equal("if_w_lt_200/c_fill,h_120,w_80/if_else/c_fill,h_90,w_100", sTransform);//, "should be proper transformation string");
        }
        ...
        public void ChainedConditions()
        {
            ...
            Assert.Equal("if_ar_gt_3:4,c_scale,w_100", transformation.ToString());//, "passing an operator and a value adds a condition");

            transformation = new Transformation().IfCondition().AspectRatio("gt", "3:4").And().Width("gt", 100).Then().Width(50).Crop("scale");
            Assert.Equal("if_ar_gt_3:4_and_w_gt_100,c_scale,w_50", transformation.ToString());//, "should chaining condition with `and`");
            ...
            Assert.Equal("if_ar_gt_3:4_and_w_gt_100_or_w_gt_200,c_scale,w_50", transformation.ToString());//, "should chain conditions with `or`");

            transformation = new Transformation().IfCondition().AspectRatio(">", "3:4").And().Width("<=", 100).Or().Width("gt", 200).Then().Width(50).Crop("scale");
            Assert.Equal("if_ar_gt_3:4_and_w_lte_100_or_w_gt_200,c_scale,w_50", transformation.ToString());//, "should translate operators");
            ...
            Assert.Equal("if_ar_gte_3:4_and_pc_gte_100_or_pc_ne_0,c_scale,w_50", transformation.ToString());//, "should translate operators");
        }
        ...
        public void ShouldSupportAndTranslateOperators()
        {
            string allOperators =
                            ...
                            "_w_gte_0" +
                            ",e_grayscale";
            Assert.Equal(allOperators, new Transformation().IfCondition()
                            .Width("=", 0).And()
                            .Height("!=", 0).Or()
                            .AspectRatio("<", "0").And()
                            .PageCount(">", 0).And()
            ...
            Assert.Equal(allOperators, new Transformation().IfCondition("w = 0 && height != 0 || aspectRatio < 0 and pageCount > 0 and faceCount <= 0 and width >= 0")
                        .Effect("grayscale")
                        .ToString());
        }
        ...
        public void EndIf2()
        {
            ...
            Assert.Equal("if_w_gt_100_and_w_lt_200/c_scale,w_50/if_end", transformation.ToString());//, "should serialize to 'if_end'");

            transformation = new Transformation().IfCondition().Width("gt", 100).And().Width("lt", 200).Then().Width(50).Crop("scale").EndIf();
            Assert.Equal("if_w_gt_100_and_w_lt_200/c_scale,w_50/if_end", transformation.ToString());//, "force the if clause to be chained");
            ...
            Assert.Equal("if_w_gt_100_and_w_lt_200/c_scale,w_50/if_else/c_crop,w_100/if_end", transformation.ToString());//, "force the if_else clause to be chained");
        }
    }
}
